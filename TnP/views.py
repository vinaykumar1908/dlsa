...
from TnP.forms import registerStockRecievedForm, registerStockOnLoan, registerStockDispatchSicklineform, registerStockDispatchedYardform, registerStockDispatchedTrainDutyform
...
def dispatchROH(request):
    if request.method=='POST':
        form = registerStockOnLoan(request.POST)
        if form.is_valid():
            itemName = request.POST.get('Item')
            stockDispatchedROH = form.cleaned_data['stockDispatched']
            StaffName2 = request.POST.get('StaffName')
            StaffMember = Staff.objects.get(StaffName=StaffName2)
            print(itemName)
        ...
        print(cur.updateTime) 
        cur.save()
        added = models.registerStockOnLoan(Item=itemName,StaffName=Staff.objects.get(StaffName=StaffName2), PL_Number=curPLNumber, stockDispatched=stockDispatchedROH)
        print(added.Item)
        print(added.PL_Number)
